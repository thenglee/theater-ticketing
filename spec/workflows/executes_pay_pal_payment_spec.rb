...
describe ExecutesPayPalPayment, :vcr, :aggregate_failures do
  describe "successful credit card purchase" do
    let!(:ticket_1) { instance_spy(Ticket, status: "pending", price: Money.new(1500), id: 1) }
    let!(:ticket_2) { instance_spy(Ticket, status: "pending", price: Money.new(1500), id: 2) }
    let!(:ticket_3) { instance_spy(Ticket, status: "pending", price: Money.new(1500), id: 3) }
