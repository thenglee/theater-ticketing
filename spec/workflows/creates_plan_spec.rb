...
RSpec.describe CreatesPlan, :vcr, :aggregate_failures do
  ...
  it "creates a plan" do
    workflow = CreatesPlan.new(
        remote_id: "basic_monthly_#{Time.now.to_i}", product: { name: "Basic Monthly" },
        nickname: "Basic Monthly", price_cents: 2000, interval: "month", tickets_allowed: 1,
    ...
    expect(workflow.plan.remote_plan).to have_attributes(
      id: a_string_starting_with("basic_monthly"), amount: 2000, nickname: "Basic Monthly",
