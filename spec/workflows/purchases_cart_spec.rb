...
describe PurchasesCart do
  describe "successful credit card purchase" do
    ...
    let(:stripe_token) { instance_spy(StripeToken, token: "tk_not_a_real_token") }
    let(:workflow) { PurchasesCart.new(user: user, purchase_amount_cents: 3000, stripe_token: stripe_token) }
    # let(:workflow) { PurchasesCart.new(user: user, purchase_amount_cents: 3000, stripe_token: instance_spy(StripeToken, token: "tk_not_a_real_token")) }
    ...
    before(:example) do
      ...
      allow(payment).to receive(:update!).with(status: "succeeded", response_id: "ch_not_an_id", full_response: a_truthy_value)
