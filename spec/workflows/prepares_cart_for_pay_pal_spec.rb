...
describe PreparesCartForPayPal, :vcr, :aggregate_failures do
  ...
  let(:discount_code_string) { nil }
    ...
    let(:workflow) { PreparesCartForPayPal.new(user: user, purchase_amount_cents: 3000, expected_ticket_ids: "#{ticket_1.id} #{ticket_2.id}", payment_reference: "reference", discount_code_string: discount_code_string) }
    ...
    context "with discount code" do
      let(:workflow) { PreparesCartForPayPal.new(user: user, purchase_amount_cents: 2250,  expected_ticket_ids: "#{ticket_1.id} #{ticket_2.id}", payment_reference: "reference", discount_code_string: discount_code_string) }
