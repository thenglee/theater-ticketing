...
RSpec.describe TaxCalculator, :aggregate_failures, :vcr do
  ...
  let(:calculator) { TaxCalculator.new(user: user, address: address, cart_id: "1",
                                       items: [
  ...
  describe "creation" do
    ...
    it "creates a transaction properly" do
      ...
      expect(transaction.cart_items.first).to have_attributes(
        index: 0, item_id: "Ticket", tic: "91083", price: 30.00, quantity: 1)
      expect(transaction.cart_items.second).to have_attributes(
        index: 1, item_id: "Processing", tic: "10010", price: 1.00, quantity: 1)
      expect(transaction.cart_items.third).to have_attributes(
        index: 2, item_id: "Shipping", tic: "11000", price: 2.00, quantity: 1)
    end

    it "handles a lookup correctly" do
      expect(calculator.tax_amount).to eq(0.20.to_money)
      expect(calculator.itemized_taxes).to eq(ticket_cents: 0,
                                              processing_cents: 0,
                                              shipping_cents: 20.0)
    end
  end
