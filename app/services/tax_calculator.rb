class TaxCalculator
  ...
  def origin
    TaxCloud::Address.new(
      address1: "1060 W. Addison", city: "Chicago", state: "IL", zip5: "60613")
  end
  ...
  def transaction
    @transaction ||= begin
      ...
      customer_id: user.id, cart_id: cart_id, origin: origin, destination: destination)
  ...
  class TicketItem < Item
    def cart_item(index)
      TaxCloud::CartItem.new(index: index, item_id: "Ticket", tic: "91083", price: price.to_f, quantity: quantity)
    end
  end

  class ShippingItem < Item
    def cart_item(index)
      TaxCloud::CartItem.new(index: index, item_id: "Shipping", tic: "11000", price: price.to_f, quantity: quantity)
    end
  end

  class ProcessingItem < Item
    def cart_item(index)
      TaxCloud::CartItem.new(index: index, item_id: "Processing", tic: "10010", price: price.to_f, quantity: quantity)
    end
  end
end
