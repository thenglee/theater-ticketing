class AffiliateForm {
  ...
  appendHidden(name, value) {
    const field = $("<input>").attr("type", "hidden")
                              .attr("name", name)
                              .val(value)
...
class AffiliateFormHandler {
  ...
  handleSubmit(event) {
    ...
    Stripe.bankAccount.createToken(this.affiliateForm.form(), BankAccountTokenHandler.handle)
    return false
  }
}
...
class BankAccountTokenHandler {
  ...
  handle() {
    if (this.isError()) {
      ...
      this.affiliateForm.appendHidden("account[external_account]", this.response.id)
