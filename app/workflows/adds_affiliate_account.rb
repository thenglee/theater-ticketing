class AddsAffiliateAccount
  ...
  def run
    Affiliate.transaction do
      @affiliate = Affiliate.create(user: user, name: user.name, country: "US", tag: Affiliate.generate_tag)
  ...
  def acquire_stripe_id
    @account = StripeAccount.new(@affiliate, tos_checked: tos_checked, request_ip: request_ip)
    account.account.id
  end
